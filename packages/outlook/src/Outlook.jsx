...
function RiskBody({ risks, foreignAnnual, ticker, pick }) {
  ...
  return (
    <>
      ...
      <div className="rc-riskbar">
        ...
        <span className="rc-riskbar-note">
          {gistState === 'loading' ? '✦ writing one-line gists…'
            ...
            : '✦ sample summaries; expand a row for its full scenario'}
        </span>
      </div>
      ...
      <p className="rc-foot">
        Generated risk statements illustrate expanding and inspecting risks. The linked sample explains the synthetic inputs.
      </p>
    </>
  );
}
...
function CatalystsBody({ c, ticker, pick }) {
  ...
  return (
    <>
      ...
      <p className="rc-foot">
        Generated guidance and event dates illustrate the catalyst workflow. No earnings transcript or company filing was downloaded.
      </p>
    </>
  );
}
...
function ConsensusBody({ c }) {
  ...
  return (
    <>
      ...
      <p className="rc-foot">
        Generated estimates and outcomes illustrate the consensus table. No IBES,
        WRDS, database, VPN or external API is used. These are fictional observations.
      </p>
    </>
  );
}
...
function WebRatingsBody({ web, pick }) {
  ...
  return (
    <>
      ...
      <p className="rc-foot">
        These ratings are generated examples. No aggregator website is contacted.
      </p>
    </>
  );
}
...
function buildSections(d, pick, web) {
  ...
  out.push({
    ...
    title: foreign ? `Risk factors — ${foreign.form} Item 3.D` : 'Risk factors — sample scenario',
    count: nRisks,
    blurb: foreign
      ? (risks?.headings
      ...
      : `${nRisks} generated risks in the sample dated ${risks.filing.filed}`,
  ...
  if (update) {
    out.push({
      ...
      title: 'Quarterly update — sample scenario',
      count: update.headings?.length || null,
      blurb: update.affirmsAnnual
        ? `Sample dated ${update.filing.filed}: the annual scenario is unchanged`
  ...
  out.push({
    ...
    title: 'Consensus — generated example',
    ...
    blurb: !ib?.available
      ...
      : !ib.covered
        ...
        : `${ib.totalRows} periods of street estimates, ${ib.rows.filter((r) => r.actual == null).length} still pending — synthetic estimates`,
